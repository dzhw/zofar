		  ...
		  console.log('VD 1');
	        $('#warning').modal('show').on('show.bs.modal', function () { //Show Warning Modal and use `show` event listener
	            ...
	            $('.confirmclosed').click(function () { //Waring Modal Confirm Close Button
	                ...
	                $('.warning-model').modal('hide'); //Hide Form Modal
//	                $('#warning').modal('hide'); //Hide Warning Modal
//	                $('#event-modalcalendarform_calendar_episodeCalendar').modal('show'); //Hide Form Modal
	            });
	        });
	    });
	  */
//	$('.closefirstmodal').click(function () { //Close Button on Form Modal to trigger Warning Modal
//		console.log('VD tetst 1');
//        $('#Warning').modal('show');//.on('show.bs.modal', function () { //Show Warning Modal and use `show` event listener
//           console.log('VD tetst 2');
//        	
//        	$('.confirmclosed').click(function () { //Waring Modal Confirm Close Button
//                $('#Warning').modal('hide'); //Hide Warning Modal
//                $('#event-modalcalendarform_calendar_episodeCalendar').modal('hide'); //Hide Form Modal
//                	console.log('VD tetst 3');
//            });
//        	$('#canceltest').click(function () { //Waring Modal Confirm Close Button
//                $('#Warning').modal('hide'); //Hide Warning Modal
//                console.log('VD tetst 4');
//            });
//    });
...
//		        endDate: new Date($( monthpicker ).attr( 'data-enddate' )),
		        startDate: new Date($(monthpicker).attr( 'data-startdate' ).substring(0, 4),$(monthpicker).attr( 'data-startdate' ).substring(5, 7)-1,$(monthpicker).attr( 'data-startdate' ).substring(8, 10)),
		        endDate: new Date($(monthpicker).attr( 'data-enddate' ).substring(0, 4),$(monthpicker).attr( 'data-enddate' ).substring(5, 7)-1,$(monthpicker).attr( 'data-enddate' ).substring(8, 10)),
...
$('#event-modalcalendarform_calendar_episodeCalendar').ready(function() {
	var inputSelector = $("#event-modalcalendarform_calendar_episodeCalendar #event-name", this);
	var typeSelector = $("#event-modalcalendarform_calendar_episodeCalendar #event-type", this);
...
$('#table-event-modalcalendarTableform_calendar_epitable').ready(function() {
	var inputSelector = $("#table-event-modalcalendarTableform_calendar_epitable #event-name", this);
	var typeSelector = $("#table-event-modalcalendarTableform_calendar_epitable #event-type", this);
//	console.log("found");
	$( this).on('show.bs.modal', function(){
		setEventVisibilityTable(inputSelector,typeSelector);
	});
...
function setEventVisibilityTable(inputSelector,typeSelector) {
...
// add placeholder for infield text
$(document).ready(function() {
	$("#form\\:pst_01\\:sc\\:rd\\:ao2\\:qo\\:aoq").attr("placeholder", "Anzahl Ausbildungen");
});

//Autoforwarding in MATRIX
$('.carousel').each(
		function() {
			var ignore = false;
			...
			if (!ignore) {
				...
				function autoforward() {
					var currentSlide = $(carouselSelector).find('.active');
					var nextSlide = currentSlide.next();
					var answerOptionSelector = $('.custom-radio', currentSlide);
					
					$(answerOptionSelector).on('mouseup keyup',function() {
						if(nextSlide.length > 0 ){
							if (window.matchMedia('(max-width: 767px)').matches)$(carouselSelector).carousel('next');
						}
					});
				}
				
				$(carouselSelector).ready(autoforward);
				$(carouselSelector).on('slid.bs.carousel', autoforward);
			}
		}
    );
