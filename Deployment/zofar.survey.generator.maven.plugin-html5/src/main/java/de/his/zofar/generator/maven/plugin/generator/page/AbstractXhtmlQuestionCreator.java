...
import com.sun.java.jsf.composite.answer.AttachedMonthpickerQuestionType;
...
abstract class AbstractXhtmlQuestionCreator extends AbstractXhtmlElementCreator {
	...
	protected void addAttachedOpenQuestion(final AttachedQuestionOpenType source, final AttachedOpenQuestionType target) throws XmlException {
		...
		if (source.isSetInfield()) {
			String message = null;
			if (PageManager.getInstance().getMojo() != null) {
				message = PageManager.getInstance().getMojo().addTextToBundle(generateUid(source)+".placeholder", source.getInfield());
			}
			
			if(message != null)target.setInfieldText(createElExpression(message));
		}
	}
	...
	protected void addAbstractAnswerOption(final AbstractLabeledAnswerOptionType source, final BaseOptionType target) throws XmlException {
		...
		if ((attachedOpensSource != null) && (attachedOpensSource.length > 0)) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (type.equals("text")) {
				...
				else if (type.equals("monthpicker")) {
					AttachedMonthpickerQuestionType attached = target.addNewAttachedMonthpickerQuestion();
					if ((attachedOpenSource).isSetMinRange())
						attached.setMinRange(attachedOpenSource.getMinRange());
					if (attachedOpenSource.isSetMaxRange())
						attached.setMaxRange(attachedOpenSource.getMaxRange());
					if (attachedOpenSource.isSetViewDate())
						attached.setViewDate(attachedOpenSource.getViewDate());
					if (attachedOpenSource.isSetDatesDisabled())
						attached.setDatesDisabled(attachedOpenSource.getDatesDisabled());
					addAttachedOpenQuestion(attachedOpenSource, attached);
					...
					LOGGER.warn("Type of open field is not known (text,nonumbers,number,grade,mail, monthpicker). Handeled as text");
					AttachedOpenQuestionType attached = target.addNewAttachedOpenQuestion();
					if (attachedOpenSource.isSetValidationMessage()) {
						String message = null;
						if (PageManager.getInstance().getMojo() != null) {
