...
class XhtmlQuestionSingleChoiceCreator extends AbstractXhtmlQuestionCreator implements IXhtmlCreator {
	...
	public void addToSection(final IdentificationalType source, final SectionType targetSection, final boolean root) throws XmlException {
		...
		Boolean isBooleanQuestion = false;
		if(source != null) {
			final QuestionSingleChoiceResponseDomainType rdc = ((QuestionSingleChoiceType) source).getResponseDomain();
			if(rdc != null) {
				QuestionSingleChoiceResponseDomainType.Type.Enum type = rdc.getType();
				if(type != null)isBooleanQuestion = type.equals(QuestionSingleChoiceResponseDomainType.Type.BOOLEAN);
			}
		}
	...
	private void createQuestion(final IdentificationalType source, final BaseQuestionType target) throws XmlException {
		...
		if(sourceQuestion != null) {
			final QuestionSingleChoiceResponseDomainType rdc = ((QuestionSingleChoiceType) sourceQuestion).getResponseDomain();
			if(rdc != null) {
				QuestionSingleChoiceResponseDomainType.Type.Enum type = rdc.getType();
				if(type != null)isBooleanQuestion = type.equals(QuestionSingleChoiceResponseDomainType.Type.BOOLEAN);
			}
	...
	private void addResponseDomain(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceType targetSingleChoice) throws XmlException {
		...
		if (source.getShowPipes()) {
			xhtmlResponseDomain.setItemClasses(PIPE_CLASSES);
		}
	...
	private void addAnswerOption(final QuestionSingleChoiceAnswerOptionType source, final SingleOptionType target) throws XmlException {
		if (source.isSetTooltip()) {
			String message = null;
			if (PageManager.getInstance().getMojo() != null) {
				message = PageManager.getInstance().getMojo().addTextToBundle(generateUid(source)+".tooltipText", source.getTooltip());
			}
			if(message != null)target.setTooltipText(createElExpression(message));
		}
