...
final class CreatorFactory {
	...
	synchronized static IXhtmlCreator newCreator(final IdentificationalType type) {
		...
		else if (EpisodesType.class.isAssignableFrom(typeClass)) {
			creator = new XhtmlEpisodesCreator();
		}
		else if (EpisodesTableType.class.isAssignableFrom(typeClass)) {
			creator = new XhtmlEpisodesTableCreator();
		}
		else{
