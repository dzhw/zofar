...
public class XhtmlComparisonQuestionCreator extends AbstractXhtmlMatrixCreator implements IXhtmlCreator {
	private static final Logger LOGGER = LoggerFactory.getLogger(XhtmlComparisonQuestionCreator.class);
	...
	private void createQuestion(final IdentificationalType source, final ComparisonQuestionType target)
			throws XmlException {
		...
		addResponseDomain(sourceQuestion.getResponseDomain(), target.addNewComparisonResponseDomain());
	}
	private void addHeaderRecursively(final XmlObject source, final org.apache.xmlbeans.XmlObject target)
			throws XmlException {
		if (source == null)
			return;
		if (target == null)
			return;
		if (de.his.zofar.xml.questionnaire.impl.TextTypeImpl.class.isAssignableFrom(source.getClass())) {
			...
			} else if ((com.sun.java.jsf.composite.composite.ComparisonUnitType.class)
		...
		} else if ((de.his.zofar.xml.questionnaire.impl.ComparisonUnitTypeImpl.class)
			...
			String visible = null;
			if (tmp.isSetVisible())
				visible = createElExpression(tmp.getVisible());
			...
			if (unit != null) {
				unit.setId(uid);
				if (visible != null)
					unit.setRendered(visible);
				if (tmp.isSetSortCondition() || tmp.isSetSortMode()) {
					final SortType sort = createSortContainer(tmp, unit.addNewSort());
	...
	/**
	 * creates a sort container if necessary. returns NULL if the source has no sort
	 * condition.
	 * 
	 * @param source
	 * @param target
	 ...
	 * @throws XmlException
	 */
	protected SortType createSortContainer(final SortableContainerType source, final SortType target)
			throws XmlException {
		...
		if (source.isSetSortMode()) {
			...
			default:
				throw new RuntimeException(
						"sort mode " + source.getSortMode() + " is either not yet implemened or not supported");
			}
			target.setMode(mode);
		}
		return target;
	}
	private void addResponseDomain(final ComparisonResponseDomainType source,
	...
	private void addItem(final XmlObject source, final ComparisonQuestionItemType target) throws XmlException {
		if ((ComparisonItemTypeImpl.class).isAssignableFrom(source.getClass())) {
			...
			for (final XmlObject object : source.selectPath(SELECT_PATH_IN_ORDER)) {
				...
				boolean flag = false;
				for (final XmlObject subobject : object.selectPath(SELECT_PATH_IN_ORDER)) {
					XmlObject[] headerTexts = XmlClient.getInstance().getByXPath(subobject, "descendant::*");
					for (final XmlObject headerText : headerTexts) {
						if ((de.his.zofar.xml.questionnaire.impl.TextTypeImpl.class)
								.isAssignableFrom(headerText.getClass())) {
							final String text = ((de.his.zofar.xml.questionnaire.impl.TextTypeImpl) headerText)
									.getStringValue();
							if (flag)
								questionTextBuffer.append(" ");
							questionTextBuffer.append(text);
							flag = true;
						}
					}
				}
				if (flag) {
					String questionText = questionTextBuffer.toString();
					final XmlObject[] childs = object.selectPath(SELECT_PATH_IN_ORDER);
					for (final XmlObject child : childs) {
						if ((de.his.zofar.xml.questionnaire.QuestionHeaderType.class)
								.isAssignableFrom(child.getClass()))
							continue;
						final XmlObject[] subchilds = child.selectPath(SELECT_PATH_IN_ORDER);
						boolean first = true;
						for (final XmlObject subchild : subchilds) {
							if ((de.his.zofar.xml.questionnaire.AbstractLabeledAnswerOptionType.class)
									.isAssignableFrom(subchild.getClass())) {
								if (first) {
									((de.his.zofar.xml.questionnaire.AbstractLabeledAnswerOptionType) subchild)
											.setLabel2(questionText);
								}
							}
						}
					}
				}
				if (!(IdentificationalType.class).isAssignableFrom(object.getClass()))
					continue;
