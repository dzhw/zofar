...
@Override
    public void beforePhase(final PhaseEvent event)
    {
        ...
        else response.addHeader("Pragma", "no-cache");
        if(response.containsHeader("Cache-Control")) {
        	response.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
        }
        else response.addHeader("Cache-Control", "no-cache, no-store, must-revalidate");
        if(response.containsHeader("Expires")) {
        	response.setHeader("Expires", "0");
        }
        else response.addHeader("Expires", "0");
        final String sessionHeader = "x-zofar-session-id";
        HttpSession session = (HttpSession) facesContext.getExternalContext().getSession(false);
        String sessionId = "UNKOWN";
        if(session != null)sessionId = session.getId();
        if(response.containsHeader(sessionHeader)) {
        	response.setHeader(sessionHeader, sessionId);
        }
        else response.addHeader(sessionHeader, sessionId);
    }
}
