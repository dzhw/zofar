...
//
//                monthDiv.textContent = monthTitle;
...
//      },
    {   key: "_zofarResponsiveRenderBody",
        value: function _zofarResponsiveRenderBody() {
        	...
        	var yearsContainer = document.createElement(coreElement);
...
//            	var monthHeadCell = document.createElement('th');
        	...
        	//yearsContainerBody.classList.add('months-container');
	        var startY = this.options.startYear+0;
	        for(var y = startY; y < startY+3;y++){
	         
//	        	var yearRow = document.createElement('tr');
	        	var yearRow = document.createElement(coreElement);
	        	yearRow.classList.add('zofar-structure-table-row');
	        	yearRow.classList.add('year-container');
	        	//yearRow.classList.add('months-container');
...
//	            	var monthBodyCell = document.createElement('td');
	                var monthBodyCell = document.createElement(coreElement);
...
//	              var monthLabelX = document.createElement('div');
    ...
    }, 
    {
        ...
        value: function _zofarResponsiveRenderDataSource() {
          ...
          if (this._dataSource != null && this._dataSource.length > 0) {
        	...
        	for (var i = 0; i < _this._dataSource.length; i++) {  		
            	var episodeStart = new Date(_this._dataSource[i].startDate.getFullYear(), _this._dataSource[i].startDate.getMonth() , 1);
              	var episodeEnd = new Date(_this._dataSource[i].endDate.getFullYear(), _this._dataSource[i].endDate.getMonth() + 1 , 0);
            
        	
              	var flag = false;
              	for(var slotIndex = 0; slotIndex < slots.length;slotIndex++){
              		var lastepisode = slots[slotIndex][slots[slotIndex].length - 1];
              		var endofslot = new Date(lastepisode.endDate.getFullYear(), lastepisode.endDate.getMonth() + 1 , 0);
              		
                	var inRange = true;
                	if(episodeStart > endofslot);
                	inRange = false;
	
                    if (inRange) {
                    	slots[slotIndex].push(_this._dataSource[i]);
                    	flag = true;
                    }
              	}
            ...
            this.element.querySelectorAll('.zofar-month').forEach(function (month) {
              ...
              //console.log(monthId+' '+firstDate+' '+lastDate);
...
//            	}
                ...
                if (monthData.length > 0) {
                	//monthData.reverse();
                    //_this._zofarResponsiveRenderDataSourceMonth(month,monthData);
                    _this._zofarRenderDataSourceMonth(month,monthData);
    ...
    {
        key: "_zofarRenderDataSource",
        value: function _zofarRenderDataSource() {
          var _this = this;
    ...
    }, 
    {
        ...
        value: function _zofarResponsiveRenderDataSourceMonth(elt,events) {
              ...
              if (weight > 0) {
                ...
                for (var i = 0; i < events.length; i++) {
                	if (window.matchMedia('(max-width: 766px)').matches) {
                        ...
                        boxShadow = " ".concat((i + 1) * weight, "px 0 0 0 ").concat(events[i].color) + " "+boxShadow;
                       ...
                       boxShadow += "inset 0 -".concat((i + 1) * weight, "px 1 0 ").concat(events[i].color);
                	}
          ...
          zofarmonths.forEach(function (month) {
  	        ...
  	        if (_this2.options.enableRangeSelection) {
  	    	    month.addEventListener('mousedown', function (e) {
  	    	    	...
  	    	    	switch (e.which) {
  		    	        case 1:
  			        	    monthStart = _this2._getZofarStartDate(e.target.closest('.zofar-month'));   	    
  			        	    monthEnd = _this2._getZofarEndDate(e.target.closest('.zofar-month')); 
  		    	            break;
  		    	        case 2:
  		    	            break;
  		    	        case 3:		                
    ...
    }, 
    {
        ...
        value: function _getZofarDate(elt) {
            var month = elt.closest('.zofar-month').dataset.monthId;
            var year = elt.closest('.zofar-month').dataset.yearId;
...
//		  startDate = new Date(startDate.getUTCFullYear(), startDate.getUTCMonth(), startDate.getUTCDate(), 0,0,0);
		  startDate = new Date(startDate.getFullYear()+'-'+(startDate.getMonth()+1)+'-'+startDate.getDate()+' 0'+(((startDate.getTimezoneOffset()/60)*(-1))+1)+':00:00');

		  //
//		  var startCorr = startDate.getTimezoneOffset()*60*1000;
//		  startDate = new Date(startDate.getTime() - startCorr);
		  console.log('getZofarEvents startDate1 : '+startDate.toISOString());
		  
          var endDate = new Date(date.getFullYear(), date.getMonth()+1, 0);
          //endDate = new Date(endDate.getUTCFullYear(), endDate.getUTCMonth(), endDate.getUTCDate(), 0,0,0);
          endDate = new Date(endDate.getFullYear()+'-'+(endDate.getMonth()+1)+'-'+endDate.getDate()+' 0'+(((endDate.getTimezoneOffset()/60)*(-1))+1)+':00:00');

//		  var endCorr = endDate.getTimezoneOffset()*60*1000;
//		  endDate = new Date(endDate.getTime() - endCorr);
