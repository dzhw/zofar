...
//    	        var adjustedDate = new Date(date.getFullYear()+'-'+(date.getMonth() < 9 ? '0': '') + (date.getMonth()+1)+'-'+(date.getDate() < 9 ? '0': '') +date.getDate()+'T0'+(((date.getTimezoneOffset()/60)*(-1))+1)+':00:00Z');
      ...
      value: function _initializeOptions(opt) {
        ...
        this.options = {
          ...
          language: opt.language != null && Calendar.locales[opt.language] != null ? opt.language : 'de',
    ...
    {   key: "_zofarResponsiveRenderBody",
        value: function _zofarResponsiveRenderBody() {
            ...
            for (var m = 1; m <= 12; m++) {
                ...
                var monthTitle = Calendar.locales[this.options.language].monthsShort[m-1];
			...
			const minMonth = Number($('#monthpickerStart').attr( 'data-startdate' ).substring(5, 7));
//			const minYear = $('#monthpickerStart').attr( 'data-startdate' ).substring(0, 4);
			const maxMonth = Number($('#monthpickerStart').attr( 'data-enddate' ).substring(5, 7));
//			const maxYear= $('#monthpickerStart').attr( 'data-enddate' ).substring(0, 4);
...
//	                }
					if ((stopY==y && maxMonth<m) || (startY==y && minMonth>m)){
						monthBodyCell.classList.add('zofar-month-disabled');
					}
...
/*				    TODO: Reihenfolge Monate*/
				    ...
				    monthId=i+j*4+1;
                           ...
                           monthBodyCell.classList.add('month');
                           monthBodyCell.classList.add('zofar-month');
                           monthBodyCell.classList.add('zofar-season-'+i);
                           if ((stopY==y && maxMonth<monthId) || (startY==y && minMonth>monthId)){
                        	   monthBodyCell.classList.add('zofar-month-disabled');
	       				   }
                           monthBodyCell.dataset.year = y.toString();
                           monthBodyCell.dataset.yearId = y.toString();
                           monthBodyCell.dataset.id = monthId;
                           monthBodyCell.dataset.monthId = monthId;
       	            	   var monthTitle = Calendar.locales[this.options.language].monthsShort[monthId-1];
       	            	   monthBodyCell.dataset.monthTitle = monthTitle;
    ...
    }, 
    {
        ...
        value: function _zofarResponsiveRenderDataSource() {
          ...
          if (this._dataSource != null && this._dataSource.length > 0) {
          	console.log("minDate : "+this.options.minDate);  
        	console.log("maxDate : "+this.options.maxDate);  
            var rangeStart = new Date(this.options.minDate.getFullYear(), this.options.minDate.getMonth() , 1);
            var rangeEnd = new Date(this.options.maxDate.getFullYear(), this.options.maxDate.getMonth() + 1 , 0);
            ...
            for (var i = 0; i < this._dataSource.length; i++) {
                ...
                var episodeEnd = new Date(episode.endDate.getFullYear(), episode.endDate.getMonth() + 1 , 0);
              	...
              	console.log("episodeStart : "+episodeStart);  
            	console.log("episodeEnd : "+episodeEnd);  
            	console.log("(episodeStart <  rangeStart) : "+(episodeStart <  rangeStart));  
            	console.log("(episodeEnd > rangeEnd) : "+(episodeEnd > rangeEnd));  

                var inRange = true;
                if(episodeStart <  rangeStart)inRange = false;
                if(episodeEnd > rangeEnd)inRange = false;
                if(!inRange)continue;
                ...
                for(var slotIndex = 0; slotIndex < slots.length;slotIndex++){
                    ...
                    for(var episodeIndex = 0; episodeIndex < slots[slotIndex].length;episodeIndex++){
                        ...
                        const diff1 = (Math.abs((episodeEnd.getTime() - slotEpisodeStart.getTime()) / (oneDay)));
                        const diff2 = (Math.abs((episodeStart.getTime() - slotEpisodeEnd.getTime()) / (oneDay)));
                        
                        if(diff1  == 1)fit = false;
                        if(diff2 == 1)fit = false;
            ...
            value: function _zofarResponsiveRenderDataSourceMonth(month,events,slotCount,slotLimit) {
              ...
              for (var slot = 0; slot < Math.min(slotCount,slotLimit);slot++) {
	        	  ...
	        	  if((typeof event === 'undefined') || (event === null)){
		        	  ...
		        	  const episodeStartDate = event.startDate;
		        	  
		        	  const episodeStartMonth = parseInt(episodeStartDate.getMonth()); 
		        	  const episodeStartYear = parseInt(episodeStartDate.getFullYear());
		        	  
		        	  const episodeEndDate = event.endDate;
		        	  
		        	  const episodeEndMonth = parseInt(episodeEndDate.getMonth());
		        	  const episodeEndYear = episodeEndDate.getFullYear();
		        	  
		        	  
		        	  startFlag = (episodeStartMonth == monthId)&&(episodeStartYear == yearId);
		        	  endFlag = (episodeEndMonth == monthId)&&(episodeEndYear == yearId);
	        	  }
	              var slotDiv = document.createElement("div");
	              slotDiv.setAttribute("data-slot", slot); 
	              var styleStr = "height:5px;display: flex;flex-direction: column;background-color:"+color+";";
	              
	              if(startFlag)styleStr = styleStr + "margin-left:15px;";
	              if(endFlag)styleStr = styleStr + "margin-right:15px;";
	              
	              slotDiv.setAttribute("style",styleStr); 
...
//              }
    ...
    },    
    {
        ...
        value: function _zofarApplyEvents() {
          var _this2 = this;
          if (typeof _this2.contextFlag === "undefined"){
        	  _this2.contextFlag = false;
          }
			...
			//alert("touch start");
  	    	    ...
  	    	    month.addEventListener('touchend', function (e) {
                    ...
                    try {
                        ...
                        //console.log(changedTouch);
                        zofarMonth = document.elementFromPoint(changedTouch.clientX, changedTouch.clientY).closest('.zofar-month');;
                    }
                    catch(e) {
			        ...
			        if($( zofarMonth ).hasClass( "zofar-month-disabled" )){
	  		    	    ...
	  		    	    switch (e.which) {
	  		    	    	case 0:
	  		    	    		//console.log("check if context menu : "+_this2.contextFlag);
	  		    	    		if(_this2.contextFlag === true){
	  		    	    			_this2.contextFlag = false;
	  		    	    			break;
	  		    	    		}
	  		    	        ...
	  		    	        case 1:
	  			        	    ...
	  			        	    if(monthEnd === null){
		  			        	    ...
		  			        	    console.log("touchend zofarMonth : "+zofarMonth+" monthStart : "+monthStart+" monthEnd : "+monthEnd);
		  			        	    if((monthStart !== null)&&(monthEnd !== null))
		  			                _this2._triggerEvent('selectRange', {
		  			                    startDate: monthStart,
		  			                    endDate: monthEnd,
...
//            });
    ...
    }, 
    {
        ...
        value: function _zofarOpenContextMenu(elt) {
          ...
          //console.log(elt);
          //console.log(elt.classList.contains('zofar-season-3'));
          const alignLeft = (elt.classList.contains('zofar-season-3') || elt.classList.contains('zofar-season-2'));
          ...
          for (var i = 0; i < events.length; i++) {
            ...
            this._renderContextMenuItems(eventItem, this.options.contextMenuItems, events[i],alignLeft);
          ...
          if (contextMenu.children.length > 0) {
            ...
            if(alignLeft){
                contextMenu.style.left = position.left - 70 + 'px';
            }
            else{
      ...
      },
    {
      ...
      value: function _openContextMenu(elt) {
        ...
        for (var i = 0; i < events.length; i++) {
          ...
          this._renderContextMenuItems(eventItem, this.options.contextMenuItems, events[i],false);
     ...
     },
    {
      ...
      value: function _renderContextMenuItems(parent, items, evt,leftAligned) {
        var subMenu = document.createElement('div');
        subMenu.classList.add('submenu');
        if(leftAligned)subMenu.classList.add('open-left');
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (items[i].items && items[i].items.length > 0) {
            ...
            icon.classList.add('arrow');
            icon.innerHTML = "&rsaquo;";
            if(!leftAligned)menuItemContent.appendChild(icon);
            //menuItemContent.appendChild(icon);

            this._renderContextMenuItems(menuItem, items[i].items, evt,leftAligned);
          }
